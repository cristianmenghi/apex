      ...
      try {
        ...
        if (remoteBenchmarkPath) {
          try {
            ...
            console.log(
              `[${benchmarkName}] ⚠️  Docker stop failed (may not have started): ${
        ...
        while (attempts < 10) {
          try {
            ...
            console.log(
              `[${benchmarkName}] ⚠️  Refresh failed (attempt ${
                attempts + 1
              }): ${
        ...
        try {
          ...
          console.error(
            `[${benchmarkName}] ⚠️  Failed to delete sandbox: ${
...
function generateMarkdownSummary(summary: BenchmarkSummaryReport): string {
  ...
  lines.push(
    `- Flags Detected: ${summary.flagsDetected}/${successful} (${
      successful > 0
...
export async function runMultipleBenchmarks(
  ...
  console.log(
    `Flags Detected: ${flagsDetected}/${successfulResults.length} (${
